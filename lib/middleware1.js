...
function middleware (options = {}) {
    ...
    /**
     * 同一个条件添加多个中间件
     * @param {function array} midcalls
     * @param {array|string|object} cond
     */
    mw.moreAdd = function (midcalls, groupTable, cond) {
        for(let i=0; i < midcalls.length; i++) {
            if (typeof midcalls[i] !== 'function'
                || midcalls[i].constructor.name !== 'AsyncFunction')
            {
                throw new Error(`middleware function must use async`);
            }
            mw.add(midcalls[i], groupTable, cond);
        }
    };
