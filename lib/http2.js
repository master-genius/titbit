...
class httpt {
    ...
    context () {
        ...
        ctx.retJSON = (data) => {
            ctx.setHeader('content-type', 'text/json; charset=utf8');
            ctx.res.encoding = 'utf8';
            ctx.res.body = typeof data === 'string' ? data : JSON.stringify(data);
        };

        ctx.retString = (data) => {
            ctx.setHeader('content-type', 'text/plain; charset=utf8');
            ctx.res.encoding = 'utf8';
            ctx.res.body = data;
        };

        ctx.retHTML = (data) => {
            ctx.setHeader('content-type', 'text/html; charset=utf8');
            ctx.res.encoding = 'utf8';
            ctx.res.body = data;
        };

        ctx.retBinary = (data, ctype = 'application/otect-stream') => {
            ctx.setHeader('content-type', ctype);
            ctx.res.encoding = 'binary';
            ctx.res.body = data;
        };
