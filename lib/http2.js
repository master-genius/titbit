...
class httpt {
  ...
  onStream () {
    ...
    var callback = (stream, headers) => {
      ...
      let streamFailed = (err) => {
        if (stream && !stream.destroyed) {
          stream.destroy(err);
        }
      };

      stream.on('error', streamFailed);
      //stream.on('aborted', streamFailed);
      ...
      if (self.config.globalLog) {
        ...
        stream.on('close', () => {
          if (stream && !stream.destroyed && stream.rstCode === http2.constants.NGHTTP2_NO_ERROR) {
            self.logger({
