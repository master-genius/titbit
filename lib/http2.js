  ...
  */
  run (port, host) {
    ...
    serv.on('sessionError', (err, sess) => {
      ...
      self.config.debug && console.error('--DEBUG--SESSION-ERROR--:',err);
    ...
    serv.on('tlsClientError', (err, tls) => {
      ...
      self.config.debug && console.error('--DEBUG--TLS--CLIENT--:', err);
    ...
    serv.on('secureConnection', (sock) => {
      ...
      sock.on('error', err => {
        self.config.debug && console.error('--DEBUG--CONNECTION--', err);
      });
