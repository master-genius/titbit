    ...
    context () {
        ...
        ctx.setHeader = function(nobj, val = null) {
            if (typeof nobj === 'string' && val != null) {
                ctx.resHeaders[nobj] = val;
            } else if (typeof nobj === 'object') {
                for(let k in nobj) {
                    ctx.resHeaders[k] = nobj[k];
                }
            }
        };
    
        ctx.status = function(stcode = null) {
            if(stcode === null) {return parseInt(ctx.resHeaders[':status']);}
            ctx.resHeaders[':status'] = (typeof stcode == 'string' 
        ...
        ctx.sendJSON = (data, encoding = 'utf8') => {
            ctx.send(data, encoding, 'text/json; charset=utf-8');
        };

        ctx.sendHTML = (data, encoding = 'utf8') => {
            ctx.send(data, encoding, 'text/html; charset=utf-8');
        };
