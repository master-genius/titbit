...
class http1 {
  ...
  onRequest () {
    ...
    var callback = (req, res) => {
      ...
      req.on('error', (err) => {
        self.requestError(err, req);
      });

      res.on('error', (err) => {
        self.requestError(err, res);
      });
      ...
      if (self.config.globalLog) {
        ...
        res.on('finish', () => {
          ...
          if (!req || req.destroyed || !res || res.destroyed) {
            return;
          }
  ...
  middleware () {
    ...
    return async (ctx, next) => {
      await new Promise((rv, rj) => {
        ...
        ctx.request.on('end',() => {
          if (ctx.request.destroyed || ctx.request.aborted || !ctx.response.writable) {
            rj();
          } else {
            rv();
          }
