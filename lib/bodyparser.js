...
class bodyparser {
  ...
  */
  ...
  parseSingleFile (ctx, start_ind, end_ind) {
    ...
    if (header_data.indexOf('filename="') > 0 && header_data.indexOf('\r\nContent-Type') >= 0)
    {
      let name = '';
      ...
      let name_end_ind = header_data.indexOf('"; filename=', name_start_ind);

      if (name_end_ind < 0) {
        name_end_ind = header_data.indexOf('";filename=', name_start_ind);
      }
      ...
      let filename = header_data.substring(filename_start, filename_end);

      file_post.filename = filename.trim();
      ...
      file_post['content-type'] = header_data.substring(ctypeind, ctypeend).trim();
      ...
      if (name !== '') {
        ctx.body[ name ] = ctx.rawBody.toString('utf8', file_post.start, file_post.end);
      }
