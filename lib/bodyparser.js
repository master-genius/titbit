...
bodyParser.pregUpload = /multipart.* boundary.*=/i;
bodyParser.formType = 'application/x-www-form-urlencoded';

bodyParser.checkUploadHeader = function(headerstr) {
    if (bodyParser.pregUpload.test(headerstr)) {
        return true;
    }
    return false;
};

bodyParser.middleware = async (ctx, next) => {
    if ((ctx.method == 'POST' || ctx.method == 'PUT' || ctx.method == 'DELETE')
        && ctx.rawBody.length > 0)
    {
        if (bodyParser.checkUploadHeader(ctx.headers['content-type'])) {
            ctx.isUpload = true;
            bodyParser.parseUploadData(ctx, ctx.app.config.maxFiles);
        } else if (ctx.headers['content-type']
            && ctx.headers['content-type'].indexOf(bodyParser.formType) >= 0)
        {
            ctx.bodyparam = qs.parse(Buffer.from(ctx.rawBody,'binary').toString('utf8'));
        } else {
