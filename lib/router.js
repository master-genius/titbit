...
function router_group (grp_name, callback, app=null, prefix=true) {
  ...
  let makeOptions = (name) => {
      let opts = {group: grp_name}
      if (name) {
  ...
  let pre_path = ''
  let ds = ''
  if (prefix && prefix_regex.test(grp_name)) {
    pre_path = grp_name.replace(/\/{2,}/g, '/')
    ds = '/'
...
class router {
  ...
  */
  ...
  group(grp_name, callback, prefix=true) {
    ...
    return this.__group__(grp_name, callback, subapp, prefix);
  }
