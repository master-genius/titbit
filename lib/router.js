 ...
 * 而这样的设计，就可以方便和中间件机制结合，扩展模块可以更好的对中间件按照路由分组进行处理。
 */
    ...
    */
    ...
    addPath (api_path, method, callback, name = '') {
        if (typeof callback !== 'function'
            ...
            throw new Error(
              `${method} ${api_path}: callback must use async statement（回调函数必须使用async声明）`
            );
        }
    ...
    findPath (path, method) {
        ...
        var args = {};
        var r = null;
        for (let i=0; i<this.argsRoute[method].length; i++) {
