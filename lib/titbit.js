...
var titbit = function (options = {}) {
    ...
    if (this.config.globalLog) {
        this.globalLog = (this.config.http2) ? globalLog.http2 : globalLog.http1;
    }
};
...
titbit.prototype.onRequest = function (req, res) {
    ...
    var callback = (req, res) => {
        ...
        if (the.config.globalLog) {
            res.on('finish', () => {
                the.globalLog(req.method, {
                    status : res.statusCode,
                    ip : remote_ip,
                    link: `${the.config.https?'https:':'http:'}//${req.headers['host']}${req.url}`
                });
            });
        }
        ...
        var findRouter = the.router.findRealPath(urlobj.pathname, req.method);
        if (findRouter === null) {
            res.statusCode = 404;
            res.end(the.config.pageNotFound);
            return ;
        }
...
titbit.prototype.onStream = function () {
    ...
    var callback = (stream, headers) => {
        ...
        if (the.config.globalLog) {
            stream.on('close', () => {
                the.globalLog(headers, stream.sentHeaders, remote_ip);
            });
        }
    
        var urlobj = url.parse(headers[':path'], true);
        if (urlobj.pathname == '') { urlobj.pathname = '/'; }
        var findRouter = the.router.findRealPath(urlobj.pathname, headers[':method']);
        if (findRouter === null) {
            stream.respond({':status': '404'});
            stream.end(the.config.pageNotFound);
            return ;
        }
