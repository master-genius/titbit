...
var titbit = function (options = {}) {
  ...
  for(let k in options) {
    switch (k) {
      ...
      case 'deny':
        optionsCheck(k, options[k], this.limit, {type: 'array'});
        break;
...
titbit.prototype.run = function(port = 2368, host = '0.0.0.0') {
  ...
  this.config.parseBody && this.add(this.bodyparser.middleware());
  ...
  '_eaddr' : {
    ...
    callback : (w, msg, handle = undefined) => {
      let errmsg = '端口已被使用，请先停止正在运行的进程。\n在Linux/Unix上，可通过\n\t'
        ...
        +' 或\n\tss -utlp\n查看相关进程。在Windows上通过任务管理器查找并结束相关进程。';
...
titbit.prototype.workerEventHandle = function () {
  cluster.on('listening', (worker, addr) => {
    ...
    let onerr = err => {
      this.config.debug && console.error('--DEBUG-WORKER-ERR:',err);
    };
