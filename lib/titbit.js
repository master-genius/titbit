 ...
 *      这在网络不好的情况下，容易导致一些套接字被长期占有，默认值为0 和node一致。
...
var titbit = function (options = {}) {
  ...
  this.config = {
    ...
    //在HTTP/1.1中，不需要考虑此选项，在 HTTP/2 中，可以设置此选项做精确的设定。
    socketTimeout : 8000,
  ...
  for(let k in options) {
    switch (k) {
      ...
      /* case 'socketTimeout':
        this.limit.timeout = options.socketTimeout; break; */
  ...
  this.limit.timeout = this.config.timeout + 50;
  this.config.socketTimeout = this.limit.timeout;
  
  if (options.socketTimeout !== undefined 
    && typeof options.socketTimeout === 'number'
    && options.socketTimeout >= this.config.timeout)
  {
    this.limit.timeout = options.socketTimeout;
    this.config.socketTimeout = options.socketTimeout;
  }
...
titbit.prototype.run = function(port = 2022, host = '0.0.0.0') {
      ...
      c.res.body = `<!DOCTYPE html><html>
        ...
        <body>
          <div style="width:86%;margin:auto;margin-top:1rem;color:#4a4a4f;">
            <h1>titbit</h1><h3>success</h3></div>
        </body>
      </html>`;
    });
  }
  ...
  if (this.config.useLimit) {
    var connlimit = new this.connfilter(this.limit, this.rundata);
    ...
    this.on('connection', (sock) => {
      ...
      sock.setTimeout(this.limit.timeout, () => {
        if (!sock.destroyed) {
          sock.destroy();
        }
      });
...
//注意这是在运行开始就要判断并解决的问题。设置值最好不低于200，也不要高于3000。
titbit.prototype.workerErrorTime = 989;
  ...
  if (cluster.isMaster) {
    ...
    cluster.on('listening', (worker, addr) => {
      worker.process.on('error', err => {
        if (the.config.debug) console.error('--DEBUG-WORKER-ERR:',err);
      });
