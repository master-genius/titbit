 ...
 * - nextMode {string} noargs|args 中间件next调用是否需要参数，默认是不需要的。
 ...
 * - logType {string} 日志类型，支持stdio、file
...
var titbit = function (options = {}) {
  ...
  this.config = {
    ...
    */
    ...
    //中间件是否需要传递参数，默认是不需要的，但是传递参数的模式性能更好，当然在比较大的请求处理测试中，性能差距并不明显。
    nextMode: 'noargs'
  };
  ...
  this.secure = {
    ...
    //而超过diemem则直接kill
    //这限制的是对heap的使用
    diemem : parseInt(this.topmem * 0.82),
    maxmem : parseInt(this.topmem * 0.78),
  ...
  this.add = function (midcall, options = {}) {
    return this.midware.add(midcall, options);
  };

  this.use = function (midcall, options = {}) {
    if (typeof options === 'object' && options.layer !== undefined) {
      //let layer = options.layer;
      delete options.layer;
      return this.addLayer(midcall, options);
    }
...
titbit.prototype.addLayer = function(hookcall, options = {}, layer = 'first') {
...
titbit.prototype.run = function(port = 2020, host = '0.0.0.0') {
  ...
  this.midware.addFinal(); //必须放在最后，用于返回最终数据。
...
titbit.prototype.daemonMessage = function () {
  ...
  if (this.config.logType == 'file') {
    try {
      ...
      logger = new console.Console(out_log, err_log);
    } catch (err) {
      console.error(err);
      logger = null;
    }
    ...
    try {
      switch(msg.type) {
       ...
       case 'route-table':
        ...
        case 'eaddr':
          console.error('端口已被使用，请先停止正在运行的进程。\n'
