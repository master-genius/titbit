...
var titbit = function (options = {}) {
   ...
   * 因为RSS包括了Buffer的占用，所以maxMem包括Buffer默认的最大限制2GB和默认V8的最大限制在1.7GB。
   * 在Node 12+版本测试heaptotal可以稍微超过2G，而对Buffer的限制则早已超过4G，
   * 在我这只有8G内存的笔记本上可以超过6G内存的使用。
   * */

  this.totalmem = os.totalmem();
  this.topmem = 2157483648;
...
titbit.prototype.fmtLoadInfo = function (type = 'tableText') {
  ...
  if (type == 'tableText') {
    ...
    let cols = '  PID       CPU      MEM,   HEAP,  USED,  EXTERNAL  CONN\n';
    let tmp = '';
    let t = '';
    let p = null;
...
titbit.prototype.showLoadInfo = function (w, id) {
  ...
  if (this.config.loadInfoFile == '--null') {
    return;
  }
