...
const context1 = require('./context1');
const context2 = require('./context2');
...
let titbit = function (options = {}) {
  ...
  this.config = {
    ...
    errorHandle: (err, errname = '') => {
      if (err) {
        if (err.code === 'HPE_INVALID_EOF_STATE' || err.code === 'ERR_REQUEST_TIMEOUT') {
          return;
        }
      }
...
titbit.prototype.__init__ = function () {
  ...
  this.context = (type = '') => {
    ...
    return this.config.http2 ? (new context2()) : (new context1());
  };
