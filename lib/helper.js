...
helper.hmacsha1 = function (key, data) {
...
helper.uuid = (prestr = '', bits = 40) => {
  let hstr = `${Date.now()}${Math.random()}${process.pid}${process.env.HOME}`;
  if (bits == 40) {
    return `${prestr}${helper.sha1(hstr)}`;
  } else {
    return `${prestr}${helper.md5(hstr)}`;
  }
};
