...
var connfilter = function (limit, rundata) {
    ...
    this.callback = (sock) => {
      ...
      //在HTTP/1.1中，超时是一致的，而在HTTP/2中，则必须有套接字超时，否则在http2模块不会自动关闭连接。
      //在HTTP/2中，因为session.close不会关闭套接字，只是不再接收请求，但是也不能直接销毁套接字，有可能还有stream没有结束。
      //无论如何，在连接建立时，设置超时让其超时关闭，避免各种错误导致请求错误但套接字不关闭的情况。
       ...
       * */
      if (the.limit.deny.length > 0 && the.limit.deny.indexOf(sock.remoteAddress)>=0)
      {
        sock.destroy();
        return false;
      }
      ...
      //检测是否超过最大连接数限制。
      if (the.limit.maxConn > 0 && the.rundata.conn > the.limit.maxConn)
      {
        sock.destroy();
        return false;
      }
      ...
      if (the.limit.maxIPRequest > 0 && the.limit.allow.indexOf(remote_ip) < 0) {

        if (the.iptable.has(remote_ip)) {

          ipcount = this.iptable.get(remote_ip);

          if (ipcount >= the.limit.maxIPRequest) {
            sock.destroy();
            return false;
          } else {
        ...
        } else if (the.iptable.size >= the.limit.maxIPCache) {
            /** 
