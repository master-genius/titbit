...
class midCore {
    ...
    add(midcall, groupTable, options = {}) {
        ...
        var self = this;
        var makeRealMid = function(prev_mid, grp) {
          if (method === null && pathname === null) {
            return async (ctx) => {
              await midcall(ctx, self.mid_group[grp][prev_mid]);
            };
          }

          return async (ctx) => {
            if (method !==null && method.indexOf(ctx.method) < 0) {
              return await self.mid_group[grp][prev_mid](ctx);
            }
            if (pathname !== null && pathname.indexOf(ctx.name) < 0) {
              return await self.mid_group[grp][prev_mid](ctx);
            }
            return await midcall(ctx, self.mid_group[grp][prev_mid]);
          };
        };
        ...
        if (group) {
            ...
            this.mid_group[this.globalKey].push(makeRealMid(last, this.globalKey));
        }
        return this;
    }
